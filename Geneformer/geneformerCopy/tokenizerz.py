...
class TranscriptomeTokenizerz:
    ...
    def tokenize_data(
        ...
        **Parameters:**
        ...
        tokenized_cells, cell_metadata = self.tokenize_files(
            Path(data_directory), file_format, adatain=adatain,
        )
    ...
    def tokenize_files(
        self, data_directory, file_format: Literal["loom", "h5ad"] = "loom", adatain=[],
    ):
        tokenized_cells = []
        ...
        if len(adatain)>0:
            file_found = 1
            print(f"Tokenizing ...")
            file_tokenized_cells, file_cell_metadata = self.tokenize_anndata('', adatain=adatain)
    ...
    def tokenize_anndata(self, adata_file_path, target_sum=10_000, adatain=[]):
    ...
    def create_dataset(
        ...
        def format_cell_features(example):
            # if len(example)<5 or len(example)>4000:
            #     print(len(example))
            try:
